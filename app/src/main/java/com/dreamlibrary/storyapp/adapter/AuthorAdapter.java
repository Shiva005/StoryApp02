...
public class AuthorAdapter extends RecyclerView.Adapter {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        ...
        if (holder.getItemViewType() == VIEW_TYPE_ITEM) {
            ...
            Glide.with(activity)
                    .load(authorLists.get(position).getAuthor_image())
                    .placeholder(R.drawable.placeholder_portable)
                    .centerCrop()
                    .into(viewHolder.imageView);

            viewHolder.constraintLayout.setOnClickListener(v -> {
                method.onClickAd(position, type, authorLists.get(position).getAuthor_id(), "", authorLists.get(position).getAuthor_name(), "", "", "");
            });
            ...
            if (adOption.conAdView.getChildCount() == 0) {
                ...
                else if (authorLists.get(position).getNative_ad_type().equals("facebook")) {
                ...
                else if (authorLists.get(position).getNative_ad_type().equals("startapp")) {
                ...
                else if (authorLists.get(position).getNative_ad_type().equals("applovins")) {
                    LayoutInflater inflater = LayoutInflater.from(activity);
                    FrameLayout nativeAdLayout = (FrameLayout) inflater.inflate(R.layout.activity_native_max_template, adOption.conAdView, false);
                    MaxNativeAdLoader nativeAdLoader = new MaxNativeAdLoader(authorLists.get(position).getNative_ad_id(), activity);
