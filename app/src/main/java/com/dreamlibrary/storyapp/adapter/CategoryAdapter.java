...
//            viewHolder.constraintLayout.setLayoutParams(params);

//            viewHolder.constraintLayout.setLayoutParams(new CardView.LayoutParams(CardView.LayoutParams.MATCH_PARENT, CardView.LayoutParams.WRAP_CONTENT));
//            viewHolder.constraintLayout.setLayoutParams(new CardView.LayoutParams(CardView.LayoutParams.MATCH_PARENT, columnWidth / 2));
            ...
            Glide.with(activity).load(categoryLists.get(position).getCat_image_thumb())
                    ...
                    .into(viewHolder.cirImage);

            viewHolder.constraintLayout.setOnClickListener(v -> {
                method.onClickAd(position, type, categoryLists.get(position).getCid(), "", categoryLists.get(position).getCategory_name(), "", "", categoryLists.get(position).getSub_cat_status());
            });

        } else if (holder.getItemViewType() == VIEW_TYPE_Ad) {
            AdOption adOption = (AdOption) holder;
            if (adOption.conAdView.getChildCount() == 0) {
                switch (categoryLists.get(position).getNative_ad_type()) {
                    case "admob":
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        private RoundedImageView cirImage;
