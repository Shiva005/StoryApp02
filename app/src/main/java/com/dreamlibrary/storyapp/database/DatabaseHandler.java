package com.dreamlibrary.storyapp.database;
...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    private static final String KEY_PDF_BOOK_TOTAL_PAGE = "pdf_total_page";
    ...
    // Remove epub in database
    public void removeEpub(String id) {
        SQLiteDatabase db = this.getWritableDatabase();
        // remove row
        db.delete(TABLE_NAME_EPUB,  KEY_EPUB_BOOK_ID + "=" + id, null);
        db.close();
    }
    ...
    public void addPdf(String id, int lastReadPosition, int totalPage) {
        SQLiteDatabase db = this.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(KEY_PDF_BOOK_ID, id);
        values.put(KEY_PDF_BOOK_LAST_READ_POSITION, lastReadPosition);
        values.put(KEY_PDF_BOOK_TOTAL_PAGE, totalPage);
    ...
    public void addTotalPage(String id, int totalPage) {
        SQLiteDatabase db = this.getWritableDatabase();
    ...
    public int getPdfTotalPage(String id) {

        SQLiteDatabase db = this.getWritableDatabase();
        @SuppressLint("Recycle") Cursor cursor = db.query(TABLE_NAME_PDF, new String[]{KEY_PDF_BOOK_ID,
                        KEY_PDF_BOOK_TOTAL_PAGE}, KEY_PDF_BOOK_ID + "=?", new String[]{String.valueOf(id)},
                null, null, null, null);
        if (cursor.moveToFirst()) {
            return cursor.getInt(cursor.getColumnIndex(KEY_PDF_BOOK_TOTAL_PAGE));
        }
    ...
    // Updating pdf in database
    public void updateTotalPdf(String id, int totalPage) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(KEY_PDF_BOOK_TOTAL_PAGE, totalPage);
        // updating row
